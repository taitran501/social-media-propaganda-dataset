...
if not path.exists(output_dir):
    try:
        makedirs(output_dir)
        print("\x1b[32m[*]\x1b[0m Created output directory successfully.")
    except Exception as e:
        print(f"\x1b[31m[!]\x1b[0m Could not create output directory: {e}")
        output_dir = path.join(cur_dir, "..")

# Set paths for CSV and Excel files
csv_path = path.join(output_dir, "YoutubeComments.csv")
excel_path = path.join(output_dir, f"YoutubeComments_{d.timestamp(d.now())}.xlsx")

# Initialize prompt for ANSI escape sequences
system("")
...
wb = Workbook()
ws = wb.active
ws.title = "YouTube Comments"

# Add column headers for YouTube comments (same as Facebook format)
